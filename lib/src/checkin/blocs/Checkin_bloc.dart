...
class CheckinBloc extends Bloc<CheckinEvent, CheckinState> {
  ...
  CheckinBloc(
    ...
    this.updateQueueUsecase,
  ) : super(LoadingCheckinState()) {
    on<FetchAllQueuesCheckinEvent>(_fetchAllQueues, transformer: restartable());

    on<GenerateOrderCheckinEvent>(_generateOrder, transformer: restartable());

    on<PrintOrderCheckinEvent>(_printOrder, transformer: restartable());
  }

  _fetchAllQueues(
      FetchAllQueuesCheckinEvent event, Emitter<CheckinState> emit) async {
    await emit.onEach(getAllQueuesUsecase.call(),
        onData: (List<QueueEntity> data) {
      emit(SuccessCheckinState(data));
    });
  }

  _generateOrder(
      GenerateOrderCheckinEvent event, Emitter<CheckinState> emit) async {
